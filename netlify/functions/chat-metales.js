// netlify/functions/chat-metales.js
// Backend seguro para Netlify Functions: la clave OpenAI vive en variables de entorno, nunca en index.html.
...
async function askOpenAI(question, selectedSources) {
  const apiKey = process.env.OPENAI_API_KEY;
  if (!apiKey) {
    return 'No hay clave de IA configurada en el servidor. La web puede seguir funcionando con el chatbot local, pero para usar IA segura hay que añadir OPENAI_API_KEY como variable de entorno en Netlify.';
  }
...
export const handler = async (event) => {
  ...
  if (event.httpMethod === 'GET') {
    return json(200, {
      ok: true,
      message: 'Función Netlify activa. Usa POST desde la web para consultar la IA segura.',
